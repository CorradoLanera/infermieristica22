...
meta() %>%
  meta_general(
    description = "Modulo di EPIDEMIOLOGIA E STATISTICA del corso in Management, Medicina Legale, Epidemiologia e Statistica per il corso di laurea in INFERMIERISTICA",
    generator = "xaringan and remark.js"
  ) %>% 
  meta_name("github-repo" = "CorradoLanera/infermieristica22") %>% 
  meta_social(
    title = "Management, Medicina Legale, Epidemiologia e Statistica",
...
$$\mbox{var}(X)=\sum_r(x_r-\mu)^2P(X=x_r)=\sigma^2$$

vale anche

$$\sigma^2=E(X-\mu)=\left(\sum_rx_r^2P(X=x_r)\right)-\mu^2$$
]
.panel[.panel-name[Esempio]
...
$$0^2(0.129)+\ldots+6^2(0.017)-2.038^2=`r round(var_otiti,3)`$$
...
$$P(X=k)=\binom{n}{k}p^k(1-p)^{n-k} = \binom{n}{n-k}p^{n-k}(1-p)^{k}=P(X=n-k)$$
]
...
$$P(X=2)=\binom{5}{2}0.51^2(0.49)^{5-2}=\frac{5 \times 4}{2 \times 1}0.51^2(0.49)^{5-2}=`r round(pbin,3)`$$
...
Si supponga che il numero di morti attribuite alla febbre tifoide in un anno sia una Poisson con parametro $\mu = 4.6$. Si cerca la probabilità su un periodo di 6 e 3 mesi che avvengono, per esempio,  zero eventi o almeno 6
...
- circa il 95% dell'area cade tra circa $-2$ e $2$
- circa il 99% dell'area cade tra circa $-2.5$ e $2.5$
...
.panel[.panel-name[Soluzione]
$$\begin{split}P(-1.5 \leq X \leq 1.5)& = 1 - (P(X \leq -1.5) + P(X \geq 1.5))\\& = 1 - 2*P(X \leq -1.5)\\& = 1 - 2 * 0.0668\\& = 0.8664\end{split}$$
...
$$\begin{split}P(a \leq X \leq b)& = P \left ( \frac{a-\mu}{\sigma} \leq Z \leq \frac{b-\mu}{\sigma}\right )\\& = P \left (Z \leq \frac{b-\mu}{\sigma}\right ) - P \left (Z \leq \frac{a-\mu}{\sigma}\right )\\& = \Phi[(b-\mu)/\sigma]-\Phi[(a-\mu)/\sigma]\end{split}$$
...
$$\begin{split}P(90 \leq X \leq 95)& = P \left ( \frac{90-80}{12} \leq Z \leq \frac{95-80}{12}\right )\\& = \Phi[1.25]-\Phi[0.83]\\& = 0.8944-0.7967\\& = 0.098\end{split}$$
]
...
[https://bit.ly/infermieristica22-rscloud](https://bit.ly/infermieristica22-rscloud)
...
- [**RStudio Cloud**](https://bit.ly/infermieristica22-rscloud): [https://bit.ly/infermieristica22-rscloud](https://bit.ly/infermieristica22-rscloud) gratis per voi per esplorare RStudio, le sue funzionalità, e allenarvi con R (link di iscrizione valido fino alla fine del corso). NOTA: a vostra scelta, è utilizzabile anche come _supporto_ durante l'esame.
...
Diapositive: [bit.ly/infermieristica22-slide](https://bit.ly/infermieristica22-slide)
...
**@CorradoLanera** |
[`r fontawesome::fa("calendar-check")` ](https://bit.ly/infermieristica22-ricevimento) [**bit.ly/infermieristica22-ricevimento**](https://bit.ly/infermieristica22-ricevimento)
